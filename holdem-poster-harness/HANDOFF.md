...
- **에디터**: 좌 폼(6개 필드) · 우 대형 포스터 실시간 미리보기. 포스터 글자 클릭 → 해당 입력 필드 포커스. 스타일 3종(센터/스플릿/볼드) 썸네일 토글
- **이어받기**: 날짜 자동 +7일 범프. 맥락 2(정규)와 4(부분 수정)가 이 흐름에 자연 통합됨
- **발행**: QC 스캔라인 스윕 애니메이션 → 토스트 → 홈 복귀·카드 추가. 수정 재발행은 카드 교체
...
0. **통합 앱 프로토타입 피드백 반영** — `demos/app.html` 검토 후 디자인·흐름 보강 (진행 중인 축). 다음 단계 후보: 온보딩(첫 실행 시 하우스 스타일 설정) 통합, 실제 렌더 PNG 연동.
